...
A concrete example of that limitation caught me out during the hardening pass, and I want to record it honestly rather than imply the tooling caught everything. `vitest-axe` passed on every screen, but it does not check colour contrast under jsdom — jsdom cannot compute rendered colour (`getContext` is unimplemented), so the contrast rule silently skips. A muted grey I was using for small text and for the unchecked control borders was only 2.75:1 on white, well under the 4.5:1 AA threshold, and it shipped even though the axe checks were green. It was found by measuring the ratios by hand, not by the automated checks. I fixed it by splitting the token (a darker value for light backgrounds, a lighter one for the navy app bar), auditing the rest of the palette the same way, and adding a unit test that asserts the contrast ratio of every token pair we depend on so it cannot silently regress.
